...
def _send_password_reset_email(to_email: str, reset_token: str) -> None:
    ...
    _send_resend_email(to_email, subject, text_body)
...
def login(inp: LoginIn, x_org_slug: Optional[str] = Header(default=None), db: Session = Depends(get_db), request: Request = None):
    ip = (request.client.host if request and request.client else "unknown")
    if TURNSTILE_SECRET and os.getenv("TURNSTILE_PROTECT_LOGIN", "true").lower() in ("1", "true", "yes"):
        if not _verify_turnstile(inp.turnstile_token, ip):
...
def forgot_password(inp: ForgotPasswordIn, x_org_slug: Optional[str] = Header(default=None), request: Request = None, db: Session = Depends(get_db)):
    ...
    if TURNSTILE_SECRET and not _verify_turnstile(inp.turnstile_token, ip):
        raise HTTPException(status_code=403, detail="Security verification failed. Please try again.")
    email = inp.email.lower().strip()
    u = db.execute(select(User).where(User.org_slug == org, User.email == email)).scalar_one_or_none()
    if u:
        try:
            ...
            db.commit()
            _send_password_reset_email(email, raw)
            try:
...
def founder_handoff(inp: FounderHandoffIn, x_org_slug: Optional[str] = Header(default=None), user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        if threshold_met and RESEND_INTERNAL_TO:
            _send_resend_email(RESEND_INTERNAL_TO, f"Orkio founder handoff • {inp.interest_type}", summary)
    except Exception:
        logger.exception("FOUNDER_HANDOFF_NOTIFY_FAILED")
    return {"ok": True, "escalation_id": esc.id, "score": score, "threshold_met": threshold_met, "summary": summary}
